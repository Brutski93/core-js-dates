...
function getWeekNumberByDate(date) {
  const dateO = new Date(date.getFullYear(), 0, 1);
  const b = (date - dateO) / 1000 / 60 / 60 / 24;
  const c = Math.ceil((b + dateO.getDay() - 1) / 7);
  console.log(date.getDate(), c);
  return dateO.getDay() <= 5 ? c : c - 1;
}
