...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  let dif = new Date(dateEnd) - new Date(dateStart);
...
function formatDate(date) {
  ...
  let hh = dateObj.getUTCHours();
  const amORpm = hh >= 12 ? 'PM' : 'AM';
  if (hh > 12) hh -= 12;
  const mm = dateObj.getMinutes().toString().padStart(2, '0');
  const ss = dateObj.getSeconds().toString().padStart(2, '0');
  return `${MM}/${DD}/${YY}, ${hh}:${mm}:${ss} ${amORpm}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const daysSinceStart = (date - yearStart) / 1000 / 60 / 60 / 24 + 1;
  let dif = yearStart.getDay() - 8;
  if (yearStart.getDay() === 0) dif = -1;
  if (yearStart.getDay() > 0 && yearStart.getDay() < 5)
    dif = yearStart.getDay() - 1;
  return Math.ceil((daysSinceStart + dif) / 7);
}
...
function getNextFridayThe13th(date) {
  ...
  let is13F = false;
  if (today < 13) tempDate.setMonth(tempDate.getMonth() - 1);
  while (!is13F) {
    tempDate.setMonth(tempDate.getMonth() + 1);
    tempDate.setDate(13);
    if (tempDate.getDay() === 5) is13F = true;
  }
  return tempDate;
}
...
function dateObjectToStr(date) {
  const DD = date.getDate().toString().padStart(2, '0');
  const MM = (date.getMonth() + 1).toString().padStart(2, '0');
  return `${DD}-${MM}-${date.getFullYear()}`;
}
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const start = strToDateObject(period.start);
  const end = strToDateObject(period.end);
  const arr = [];
  let i = 1;
  while (start <= end) {
    arr.push(dateObjectToStr(start));
